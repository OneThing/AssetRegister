...
record_set('assetlist',"SELECT asset_id, scotuser_name, device_name, status_name, asset_model, asset_serial FROM assets JOIN scotusers ON scotuser_id = asset_user JOIN status ON status_id = asset_status JOIN devices ON asset_device = device_id WHERE scotuser_department = '".$_GET['dept']."' ORDER BY scotuser_name");
record_set('deptdetails',"SELECT department_id, department_name FROM departments WHERE department_id = '".$_GET['dept']."'");
...
<title><?php echo $row_deptdetails['department_name']; ?></title>
...
<h2><?php echo $row_deptdetails['department_name'];?></h2>
