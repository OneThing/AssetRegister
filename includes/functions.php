...
global ${"totalRows_$name"};
	...
	${"query_$name"} = "$query";
	${"$name"} = sqlsrv_query($connection,${"query_$name"}) or die(print_r( sqlsrv_errors(), true));
	${"row_$name"} = sqlsrv_fetch_array(${"$name"});
	${"totalRows_$name"} = sqlsrv_has_rows(${"$name"});
...
function select_list($id,$rs,$matcha,$matchb,$v,$display) {
	...
	} while (${"row_$rs"} = sqlsrv_fetch_array(${"$rs"})); 
...
function date_picker($name, $value, $startyear=NULL, $endyear=NULL)
{
    ...
    if($startyear==NULL) $startyear = date("Y")-20;
...
// Update History
function update_history($type, $operation, $value, $ref, $user, $parent)
{
sqlsrv_query("INSERT INTO history (history_date, history_type, history_operation, history_ref, history_pageid, history_user, history_parentid) VALUES 
